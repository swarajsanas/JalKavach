...
const NAV = [
  ...
  { id: 'map',        label: 'Map & Districts',  icon: 'fa-map-location-dot',    roles: ['admin'] },
  ...
  <div class="fade-in space-y-4">
    <div class="card p-4">
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead><tr class="border-b border-slate-100">
            <th class="th">Date</th><th class="th">Name</th><th class="th">Age/Gender</th><th class="th">Disease</th><th class="th">Village</th><th class="th">Severity</th><th class="th">Status</th><th class="th">Actions</th>
          </tr></thead>
...
function renderPatientsTable(rows) {
  ...
  tbody.innerHTML = rows.length ? rows.map(p => `
    ...
    </tr>`).join('') : `<tr><td colspan="8" class="td text-center text-slate-400 py-8">No cases reported yet</td></tr>`;
}
...
function renderWater() {
  ...
  <div class="fade-in space-y-4">
    ...
    <div class="card p-4">
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead><tr class="border-b border-slate-100">
            <th class="th">Date</th><th class="th">Village</th><th class="th">Source</th><th class="th">pH</th><th class="th">Turbidity</th><th class="th">TDS</th><th class="th">Status</th><th class="th">Actions</th>
          </tr></thead>
  ...
  tbody.innerHTML = STATE.water.length ? STATE.water.map(w => `
    <tr>
      ...
      <td class="td whitespace-nowrap">
        ${can('water', 'edit') ? `<button onclick="openWaterModal('${w.id}')" class="text-slate-500 hover:text-teal-600 mr-2"><i class="fas fa-pen"></i></button>` : ''}
        ${can('water', 'delete') ? `<button onclick="deleteDoc('water','${w.id}','water sample')" class="text-slate-500 hover:text-red-600"><i class="fas fa-trash"></i></button>` : ''}
      </td>
    </tr>`).join('') : `<tr><td colspan="8" class="td text-center text-slate-400 py-8">No water quality logs yet</td></tr>`;
}
...
function openWaterModal(id) {
  ...
  openModal(w ? 'Edit Water Sample' : 'Log Water Sample', `
    <form onsubmit="return saveWater(event)" class="space-y-3">
      <div><label class="block text-xs font-medium text-slate-600 mb-1">Village</label>
...
async function saveWater(e) {
  ...
  setLoading(true);
  try {
    if (STATE.editing.water) {
      await COL.water.doc(STATE.editing.water).update(payload);
    } else {
...
function reportColumns() {
  ...
  { label: 'Water Collected', get: r => r.waterCollected ? 'Yes' : 'No' }, { label: 'Submitted By', key: 'submittedByName' }, { label: 'Date', get: r => fmtDateTime(r.submittedAt) }];
}
  ...
  openModal(u ? 'Edit User' : 'Add User', `
    <form onsubmit="return saveUser(event)" class="space-y-3">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div><label class="block text-xs font-medium text-slate-600 mb-1">Password</label><input id="uPassword" class="input" placeholder="${u ? 'Leave blank to keep current' : 'Required'}" ${u ? '' : 'required'}></div>
      </div>
      <div class="grid grid-cols-2 gap-3">
